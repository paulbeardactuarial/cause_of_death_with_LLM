...
[Cigarette Smoking Cessation, Total and Cause-specific Mortality: A 22-Year Follow-up Study in US Male Physicians](https://pmc.ncbi.nlm.nih.gov/articles/PMC3229033/#:\~:text=Current%20smokers%20had%20significantly%20higher,23.93)%2C%20smoking%2Drelated%20cancers)
...
results_df |> 
  ...
  filter(
    category_unanimous_consensus != category_human
  ) |> 
  select(
    cause_of_death, category_human, category_unanimous_consensus
  ) |> 
  arrange(category_human, category_unanimous_consensus) |> 
  gt() |> 
  ...
  tab_spanner("Category", starts_with("category_"))  |> 
    cols_width("category_human" ~ px(200),
             "category_unanimous_consensus" ~px(200))

results_df |> dplyr::filter(if_any(starts_with("category_"), function(x) x != category_human)) |> View()

results_df |> dplyr::filter(if_any(starts_with("category_"), function(x) x != category_human & x == "none" & category_human != "none")) |> View()
