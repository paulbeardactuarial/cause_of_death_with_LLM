...
To hook up R to the API of various LLMs and get it to perform the onerous task of classifying this data for us? Can we get the LLMs to do a job that might have taken an actuary lots of time, and googling? Can we do it in a way that if circumstances change we simply press a button and update?

The Premise For this experiment I will get different LLMs to classify the ICD-10 causes of death into broader categories. The categories I have pulled from a paper online related to the cause-specific mortality impact of smoking. The paper and categories are not too important, they are just an example of what an actuary might be use. The following paper...
...
results_df |>
  ...
  tab_options(column_labels.background.color = "green3")
...
results_df |>
  ...
  arrange(desc(f1)) |> 
  gt() |> 
  fmt_percent(columns = tidyselect::any_of(c("precision", "recall", "f1")), decimals = 1) |> 
  cols_label_with(fn = function(x) stringr::str_replace(x, "_", " ") |> tools::toTitleCase()) |> 
  tab_options(column_labels.background.color = "green3")
...
results_df |> 
  ...
  filter(
    category_consensus != category_human & category_human != "none"
  ) |> 
  glimpse()
